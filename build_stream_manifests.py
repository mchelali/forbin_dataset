...
def build_stream_metadata(image: dict, metadata_lookup: dict) -> dict:
    file_name = image["file_name"]
    carton = file_name.split("/", 1)[0]
    metadata_image = metadata_lookup.get(file_name) or {}
    metadata = metadata_image.get("metadata") or image.get("metadata") or {}
    return {
        **metadata,
        "Carton": carton,
...
def make_stream_side_record(image: dict, metadata_lookup: dict) -> dict:
    file_name = image["file_name"]
    return {
        ...
        "metadata": build_stream_metadata(image, metadata_lookup),
    }
...
def build_stream_manifests(
    ...
    for image in payload.get("images", []):
        if not image.get("file_name"):
            continue
        side_record = make_stream_side_record(image, metadata_lookup)
    ...
    prediction_index = build_prediction_manifests(
        detections_source,
        output_dir,
        image_lookup,
